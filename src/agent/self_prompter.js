...
export class SelfPrompter {
    ...
    async startLoop() {
        ...
        while (!this.interrupt) {
            ...
            // [mindaxis-patch:saved-places-global-hint] MemoryBank の saved_places をヒントとして提供（再起動で消えない）
            // MemoryBank.memory は再起動で空になるが、MemoryBank.getKeys() で確認できる場合に表示
            try {
                const _spMem = this.agent.memory_bank.memory || {};
                const _spEntries = Object.entries(_spMem).filter(([k, v]) => k !== 'last_death_position' && Array.isArray(v) && v.length === 3);
                if (_spEntries.length > 0) {
                    const _spStr = _spEntries.map(([k, v]) => `"${k}"=(${Math.round(v[0])},${Math.round(v[1])},${Math.round(v[2])})`).join(', ');
                    houseHint += ' SAVED LOCATIONS: ' + _spStr + '. Use !goToRememberedPlace("<name>") to go there directly — do NOT search blindly!';
                }
            } catch(_spe) {}
            ...
            let planHint = '';
            {
                ...
                // [mindaxis-patch:water-trap-hint] 水中にいる場合は脱出を最優先
                try {
                    const _wtBot = this.agent.bot;
                    const _wtPos = _wtBot.entity && _wtBot.entity.position;
                    if (_wtPos) {
                        const _wtFeet = _wtBot.blockAt(_wtPos.floored());
            ...
            const msg = `You are self-prompting with the goal: '${this.prompt}'.${houseHint}${planHint} Your next response MUST contain a command with this syntax: !commandName. /* [mindaxis-patch:chest-management-v2] */ CHEST RULE: NEVER place chests outside your home base. RESOURCE RULE: Only use !takeFromChest when you are at home preparing to go out. When exploring far from home (50+ blocks away), do NOT return just to deposit items — keep exploring until (a) inventory has 12+ item types, (b) food runs out, or (c) it is night without a bed. Do NOT take furniture (furnace, crafting_table) from chests. A BED is OK to carry for exploration — !goToBed places it, sleeps, then picks it back up. BUILDING RULE: NEVER place cobblestone, stone, dirt, oak_planks, or other building blocks inside your house. Only place furniture (bed, chest, furnace, crafting_table, torch). FURNITURE PLACEMENT: Place furniture against the back and side walls, away from the door entrance. Keep the area near the door clear for easy entry/exit. /* [mindaxis-patch:pickaxe-rule] */ PICKAXE RULE: Before any mining or digging task (searchForBlock, collectBlocks for ores/stone), check if you have a pickaxe. If not, craft one immediately: !craftRecipe("stone_pickaxe", 1) needs 3 cobblestone + 2 sticks, or !craftRecipe("wooden_pickaxe", 1) needs 3 planks + 2 sticks. Do this BEFORE searching for blocks — a pickaxe is essential for efficient digging. /* [mindaxis-patch:villager-retry-v4] */ VILLAGER RULE: (1) NITWIT = PERMANENTLY USELESS. If !showVillagerTrades says "Nitwit", that entity can NEVER trade. Do NOT go back to them. (2) VILLAGE SAVE RULE: Do NOT call !rememberHere("village") until you have SUCCESSFULLY seen actual trade offers from a villager (!showVillagerTrades returned real offers, not an error). Nitwit areas are NOT villages worth saving. (3) ESCAPE SEQUENCE — MANDATORY: When in a nitwit area or when BLOCKED VILLAGER IDs are listed, escape in this order: FIRST try !escapeEnclosure(80) — this finds the exit from the trapped terrain using flood-fill. THEN try !goToCoordinates(112, 70, 486, 5) to return home. Do NOT use !moveAway — it fails in complex terrain. After arriving home (~112, 70, ~486), search a NEW direction far from x=360, z=860. (4) DANGER ZONE: The area near coordinates (360, 58, 861) is a lake where you drowned before. AVOID this area. Do NOT navigate within 200 blocks of x=360, z=861.${(() => { try { const _bv = this.agent.bot._blockedVillagerIds; const _tv = this.agent.bot._tempBlockedVillagers; const _bList = _bv && _bv.size > 0 ? [..._bv].join(', ') : ''; const _tNow = Date.now(); const _tList = _tv ? Object.entries(_tv).filter(([,ts]) => _tNow - ts < 3600000).map(([id]) => id).join(', ') : ''; const _parts = []; if (_bList) _parts.push(`PERMANENTLY BLOCKED (nitwit): ${_bList}`); if (_tList) _parts.push(`TEMPORARILY BLOCKED (unemployed, 1h): ${_tList}`); return _parts.length > 0 ? ` BLOCKED VILLAGER IDs — do NOT call !showVillagerTrades or navigate to these entity IDs: ${_parts.join('; ')}.` : ''; } catch(_bve) { return ''; } })()} Respond:`;
